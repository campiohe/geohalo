...
def test_batched_layouts_and_dtypes(dtype, batch_shape, layout):
    lat, lon = np.arange(4.), np.arange(6.)
    operator = Resampler.compute(lat, lon, lat, lon, method="conservative")
    ...
    np.testing.assert_array_equal(actual, values.reshape(*batch_shape, 24))
